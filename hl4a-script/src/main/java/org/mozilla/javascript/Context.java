	...
	static final Context enter(Context cx, ContextFactory factory) {
		...
		if (old != null) {
			...
			if (cx == null) {
				...
				if (cx.enterCount != 0) {
					throw new IllegalStateException("factory.makeContext() returned Context instance already " +
							"associated with some thread");
				}
				...
				if (cx.enterCount != 0) {
					throw new IllegalStateException("can not use Context instance already associated with some " +
							"thread");
				}
			}
				...
				try {
					...
					if ("Mozilla Rhino".equals(attrs.getValue("Implementation-Title"))) {
						implementationVersion =
								"Rhino " + attrs.getValue("Implementation-Version") + " " + attrs.getValue("Built-Date"
								).replaceAll("-", " ");
