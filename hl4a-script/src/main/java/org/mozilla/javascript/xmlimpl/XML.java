...
class XML extends XMLObjectImpl {
	...
	XMLList child(XMLName xmlName) {
		//    TODO    Right now I think this method would allow child( "@xxx" ) to return the xxx attribute, which is
		// wrong

		XMLList rv = newXMLList();

		//    TODO    Should this also match processing instructions?  If so, we have to change the filter and also
		// the XMLName
	...
	boolean equivalentXml(Object target) {
		...
		if (target instanceof XML) {
			//    TODO    This is a horrifyingly inefficient way to do this so we should make it better.  It may also
			// not work.
	...
	void setName(QName name) {
		...
		if (isProcessingInstruction()) {
			//    Spec says set the name URI to empty string and then set the [[Name]] property, but I understand this
			// to do the same
	...
	public String getClassName() {
		//    TODO:    This appears to confuse the interpreter if we use the "real" class property from ECMA.
		// Otherwise this code
		//    would be:
		//    return ecmaClass();
		return "XML";
