...
class XmlProcessor implements Serializable {
	...
	final Node toXml(String defaultNamespaceUri, String xml) throws org.xml.sax.SAXException {
		...
		try {
			...
			if (ignoreWhitespace) {
				//    Apparently JAXP setIgnoringElementContentWhitespace() has a different meaning, it appears from
				// the Javadoc
				//    Refers to element-only content models, which means we would need to have a validating parser and
				// DTD or schema
	...
	private void beautifyElement(Element e, int indent) {
		...
		//    We "mark" all the nodes first; if we tried to do this loop otherwise, it would behave unexpectedly (the
		// inserted nodes
