...
class DToA {
	...
	static final int
			...
			DTOSTR_FIXED = 2,                 /* Round to <precision> digits after the decimal point; exponential if
			number is large */
			...
			do {
				...
				if (j1 == 0 && ((word1 & 1) == 0)) {
					...
					if (j1 > 0) {
						...
						b = b.shiftLeft(1);
						j1 = b.compareTo(s);
						if (j1 > 0) /* The even test (|| (j1 == 0 && (digit & 1))) is not here because it messes up
						odd base output
...
//      s++;
	...
	JS_dtostr(StringBuilder buffer, int mode, int precision, double d) {
		int decPt;                                    /* Position of decimal point relative to first digit returned by
		 JS_dtoa */
...
//                DTOSTR_VARIABLE_BUFFER_SIZE(precision)));
		...
		if (mode == DTOSTR_FIXED && (d >= 1e21 || d <= -1e21))
			mode = DTOSTR_STANDARD; /* Change mode here rather than below because the buffer may not be large enough
