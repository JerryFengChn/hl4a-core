	...
	private NativeTypedArrayView js_constructor(Context cx, Scriptable scope, Object[] args) {
		if (!isArg(args, 0)) {
			...
			if ((byteLen % getBytesPerElement()) != 0) {
				throw ScriptRuntime.constructError("RangeError", "offset and buffer must be a multiple of the byte " +
						"size");
			}
...
// #generated# Last update: 2016-03-04 20:59:23 GMT
		L0:
		{
			...
			if (s_length == 3) {
				c = s.charAt(0);
				if (c == 'g') {
					if (s.charAt(2) == 't' && s.charAt(1) == 'e') {id = Id_get; break L0;}
				} else if (c == 's') { if (s.charAt(2) == 't' && s.charAt(1) == 'e') {id = Id_set; break L0;} }
			} else if (s_length == 8) { X = "subarray"; id = Id_subarray; } else if (s_length == 11) {
				X = "constructor"; id = Id_constructor;
			}
...
// #generated# Last update: 2014-12-08 17:33:28 PST
		L0:
		{
			id = 0; String X = null;
			int s_length = s.length();
			if (s_length == 6) { X = "length"; id = Id_length; } else if (s_length == 17) {
				X = "BYTES_PER_ELEMENT"; id = Id_BYTES_PER_ELEMENT;
			}
			if (X != null && X != s && !X.equals(s)) id = 0;
			break L0;
		}
