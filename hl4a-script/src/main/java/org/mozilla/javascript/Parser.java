...
public class Parser {
	...
	private AstNode parseFunctionBody(int type, FunctionNode fnNode)
		...
		try {
			if (isExpressionClosure) {
				...
				ReturnStatement n = new ReturnStatement(returnValue.getPosition(), returnValue.getLength(),
						returnValue);
	...
	private void arrowFunctionParams(FunctionNode fnNode, AstNode params, Map<String, Node> destructuring,
	...
	private AstNode objliteralProperty() throws IOException {
		...
		switch (tt) {
			...
			default:
				if (compilerEnv.isReservedKeywordAsIdentifier()
						&& TokenStream.isKeyword(ts.getString(), compilerEnv.getLanguageVersion(),
